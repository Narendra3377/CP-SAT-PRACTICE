package com.testing.util;

public class Constants {
	
	public static final String ENV="SIT";
	
	//URL's
	public static final String amazonUrl="https://www.amazon.in";
	public static final String gmailUrl="https://gmail.com";
	public static final String uploadFileUrl="http://demo.guru99.com/selenium/upload/";
	
	//Extent Report Path
//	public static final String REPORT_PATH="C:\\reports\\";
	public static final String REPORT_PATH=".\\Screenshots\\";
	
	//Web Driver paths for Windows machines
	public static final String CHROME_DRIVER_EXE="./Drivers/chromedriver.exe";
	public static final String FIREFOX_DRIVER_EXE="./Drivers/geckodriver.exe";
	public static final String IE_DRIVER_EXE="src\\test\\resources\\iedriver.exe";
	
	/*
	//Web Driver paths for Mac machines
	public static final String CHROME_DRIVER_EXE="src/test/resources/chromedriver";
	public static final String FIREFOX_DRIVER_EXE="src/test/resources/geckodriver";
	public static final String IE_DRIVER_EXE="src/test/resources/iedriver";
	*/
	
	//wordpress excel file path
	public static final String ExcelTest_XLS="src\\test\\resources\\TestData\\ExcelTest.xlsx";

}
